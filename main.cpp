...
int main () {
        ...
        ((int*)messageData)[0] = (int) sizeof(messageData) - sizeof(oscc::type::int32);
        ...
        ((int*)bundleData)[0+21] = (int) sizeof(messageData)+8 - sizeof(oscc::type::int32);
        ...
        ((int*)bundlebundleData)[0+21] = (int) sizeof(messageData)+8 - sizeof(oscc::type::int32);
        ...
        ((int*)bundlebundleData)[0+39+21] = (int) sizeof(messageData)+8 - sizeof(oscc::type::int32);
        ...
        for(pair<char*, size_t> data : testData) {
                size_t indent = 0;
                function < void(oscc::type::packet) > visitor = [&indent, &visitor, &init](oscc::type::packet &&val) {
                        using namespace std;
                        ...
                        if (holds_alternative<oscc::type::bundle>(val)) {
                                ...
                                for (const oscc::type::argument &argument: message.arguments()) {
                                        visit([&i2, &init](auto &&arg) {
                                                using T = decay_t<decltype(arg)>;
                                                if constexpr(is_same_v<T, oscc::type::int32>) {
                                                        cout << i2 << string(20,'-') << endl;
                                                        cout << i2 << "TYPE: INT 32" << endl;
                                                        cout << i2 << "VALUE: " << arg << endl;
                                                } else if constexpr(is_same_v<T, oscc::type::float32>) {
                                                        cout << i2 << string(20,'-') << endl;
                                                        cout << i2 << "TYPE: FLOAT 32" << endl;
                                                        cout << i2 << "VALUE: " << arg << endl;
                                                } else if constexpr(is_same_v<T, oscc::type::string>) {
                                                        cout << i2 << string(20,'-') << endl;
                                                        cout << i2 << "TYPE: STRING" << endl;
                                                        cout << i2 << "VALUE: \"" << arg << "\"" << endl;
                                                } else if constexpr(is_same_v<T, oscc::type::blob>) {
                                                        cout << i2 << string(20,'-') << endl;
                                                        cout << i2 << "TYPE: BLOB:" << endl;
                                                        int i = 0;
                                                        for (const char &c: (oscc::type::blob) arg) {
                                                                if (i == 4) cout << "   ";
                                                                else if (i == 8) {
                                                                        cout << "\n";
                                                                        i = 0;
