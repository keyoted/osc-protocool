...
#define EPOCH_TIME_DIFFERENCE_MS (EPOCH_TIME_DIFFERENCE_S * 1000LL)
        ...
        bool char_match(const char*& addr, const char*& ptrn) {
                if ((*addr == '\0') ^ (*ptrn == '\0')) {  // addr and ptrn are not empty or are bath empty
                        return false;
                           ...
                           (*ptrn == '?' &&
                            *addr != '/')) {  // addr == ptrn or ptrn matches any character (besides '/')
                        ...
                        if (*ptrn == '\0') {  // list at end of string
                                ...
                                if (*ptrn == '!') {  // if first char is '!' then we return true iff the char in addr is
                                                     // NOT in the list
                                        match = false;
                                        ptrn++;
                                }
                                while (*ptrn != ']' && *ptrn != '\0') {  // iterate over the character list
                                        if (                             // handle '-' sign (eg. "[a-zA-Z]")
                                                ...
                                                *(ptrn - 1) != '[' &&  // '-' at beginning of list is UB, decided no
                                                                       // special meaning
                                                !(*(ptrn - 1) == '!' &&
                                                  *(ptrn - 2) == '[')  // '-' after [! is UB, decided no special meaning
                                        ) {
                                                if (*addr >= *(ptrn - 1) &&
                                                    *addr <= *(ptrn + 1)) {  // if the number is in range
                                                        if (match) {
                        ...
                        if (*ptrn == '\0') {  // UB no closing '}'
                                        ...
                                        if (*ptrn != *addr) {                            // if characters don't match
                                                ...
                                                while (true) {                           // increment +
                                                        ...
                                                        if (*ptrn == '\0') {  // until we hit UB -- no closing '}'
                                                                ptrn = pr;
                                                                return false;
                                                        } else if (*ptrn == '}') {  // until we finish this word and
                                ...
                                if (*ptrn == '\0' ||  // UB -- no closing '}'
                                    *(ptrn - 1) ==
                                            ',' ||      // UB -- empty last word ...",}" or empty word "{"...",,"..."}"
        ...
        bool isValidAddress(std::string address) {
                ...
                return std::all_of(address.begin(), address.end(), [](char c) {
                        return isprint(c) && c != ' ' && c != '#' && c != '*' && c != ',' && c != '?' && c != '[' &&
                               c != ']' && c != '{' && c != '}';
                });
        }

        bool isValidPattern(std::string pattern) {
                enum validate_mode_ { normal, in_brackets, in_list };
        ...
        type::time getCurrentTime() {
                return type::time{std::chrono::duration_cast<std::chrono::milliseconds>(
