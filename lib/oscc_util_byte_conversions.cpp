...
#define BUNDLE_IDENTIFIER ("#bundle")
        ...
        type::packet bytesToOSC(type::blob bytes) { return bytesToOSC(bytes.data(), bytes.size()); }
                ...
                if (data.peek() == BUNDLE_IDENTIFIER[0]) {
                        ...
                        if (read::string(data) != BUNDLE_IDENTIFIER)
                                throw std::domain_error(
                                        type::string{"When a OSC packet address starts with '#' it must be \""} +
                                        BUNDLE_IDENTIFIER + "\" representing a bundle");
                        type::bundle bundle{read::time(data)};
                        while (!data.isEmpty()) bundle.push(bytesToOSC(data));
                                ...
                                for (std::size_t i = 1; i < typeTag.length(); i++) {
                                        switch (typeTag[i]) {
                                                ...
                                                case '[': {
                                                        push(type::arguments{});
                                                        stack.emplace_back(
                                                                &std::get<type::arguments>(stack.back()->back()));
                                                } break;
...
#endif
                                                default:
                                                        throw std::domain_error(std::string("Unrecognized tag: [") +
                                                                                typeTag[i] + "]");
                                        }
                                }
                                return type::packet{message};
        ...
        void OSCToBytes(const type::packet& data, std::vector<char>& bytes) {
                ...
                if (holds_alternative<type::bundle>(data)) {
                        ...
                        for (auto& content : bundle.contents()) { OSCToBytes(content, bytes); }
                } else {
                        // Is message
                                ...
                                visit(
                                        [&](auto&& arg) {
                                                ...
                                                if constexpr (is_same_v<T, type::int32>) {
                                                        ...
                                                        throw domain_error("Unknown type for argument");
                                        },
                                        argument);
                        }
                }
